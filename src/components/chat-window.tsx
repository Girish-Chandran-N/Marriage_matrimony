    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            ...
            if (result?.error === "UPGRADE_REQUIRED") {
                // Revert optimistic message and show upgrade banner
                setMessages(prev => prev.filter(m => m.id !== tempId));
                setInputText(content);
                setChatBlocked(result.message || "Upgrade your plan to send messages.");
            } else if (result?.error) {
                console.error("Failed to send:", result.error);
                // Revert optimistic message on generic error
                setMessages(prev => prev.filter(m => m.id !== tempId));
                setInputText(content);
            } else {
                // Refresh
                        ...
                        {!intentAccepted ? (
                            ...
                            <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="flex flex-col gap-3">
                                    <Button variant="outline" className="rounded-full text-slate-300 border-[#333] bg-[#121214] hover:bg-[#1f1f23] hover:text-white transition-colors h-auto py-2.5 px-4 whitespace-normal text-left justify-start shadow-sm" onClick={() => setInputText("Hi there! What does marriage mean to you?")}>
                                        What does marriage mean to you?
                                    </Button>
                                    <Button variant="outline" className="rounded-full text-slate-300 border-[#333] bg-[#121214] hover:bg-[#1f1f23] hover:text-white transition-colors h-auto py-2.5 px-4 whitespace-normal text-left justify-start shadow-sm" onClick={() => setInputText("Hey! How involved is your family in this process?")}>
                                        How involved is your family in this process?
                                    </Button>
                                    <Button variant="outline" className="rounded-full text-slate-300 border-[#333] bg-[#121214] hover:bg-[#1f1f23] hover:text-white transition-colors h-auto py-2.5 px-4 whitespace-normal text-left justify-start shadow-sm" onClick={() => setInputText("Hello! What are your core values in a relationship?")}>
                        ...
                        <div key={msg.id} className={`flex gap-3 group ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`flex flex-col max-w-[75%] md:max-w-[65%] ${isMe ? 'items-end' : 'items-start'}`}>
                                ...
                                <div className={`px-5 py-3 text-[15px] shadow-sm relative leading-relaxed ${isMe
                                    ? 'bg-gradient-to-tr from-rose-500 to-pink-600 text-white rounded-[20px] rounded-tr-sm'
                                    : 'bg-[#1f1f23] text-white rounded-[20px] rounded-tl-sm'
                                    }`}>
                                    <p>{msg.content}</p>
                ...
                <form onSubmit={handleSend} className="max-w-4xl mx-auto relative flex items-center gap-3">
                    ...
                    <div className="absolute right-14 top-1/2 -translate-y-1/2">
                        <Button
                            ...
                            size="icon"
                            disabled={!!chatBlocked || (messages.length === 0 && !intentAccepted)}
                            className="rounded-full w-10 h-10 text-slate-500 hover:text-rose-500 hover:bg-[#222] disabled:opacity-50"
                            onClick={() => alert("Premium Feature: Voice notes coming soon!")}
                        >
                    ...
                    <Button
                        type="submit"
                        disabled={!inputText.trim() || !!chatBlocked}
                        className="rounded-full w-12 h-12 p-0 flex items-center justify-center text-white shadow-lg shadow-rose-500/20 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:shadow-none bg-gradient-to-tr from-rose-500 to-pink-600"
                    >
